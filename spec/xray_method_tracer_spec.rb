  ...
  describe ".call" do
    context "引数klassesが存在する場合" do
      ...
      context "インスタンスメソッドのみ存在する場合" do
        ...
        it { expect(Bird.instance_method(:hello).source_location.first).to match(%r{methods/instance_method.rb}) }
        it { expect(Fish.instance_method(:hello).source_location.first).to match(%r{methods/instance_method.rb}) }
      end
